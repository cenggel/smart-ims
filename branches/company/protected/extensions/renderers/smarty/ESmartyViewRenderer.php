...
class ESmartyViewRenderer extends CApplicationComponent implements IViewRenderer
{
	...
	public function init(){
		...
		$this->smarty->setPluginsDir(array_merge($this->smarty->getPluginsDir(),  array(Yii::getPathOfAlias('application.extensions.renderers.smarty.plugins'))));
		
		if(!empty($this->pluginsDir)){
			//$this->smarty->plugins_dir[] = Yii::getPathOfAlias($this->pluginsDir);
			$this->smarty->setPluginsDir(Yii::getPathOfAlias($this->pluginsDir));
		}
		//print_r($this->smarty->getPluginsDir());exit;

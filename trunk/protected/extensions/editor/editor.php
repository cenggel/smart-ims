...
class Editor extends CInputWidget
{
    ...
    public function init()
    {
        ...
        }elseif($this->type === 'fckeditor'){
            $this->path = Yii::app()->getAssetManager()->publish(dirname(__FILE__).DIRECTORY_SEPARATOR."fckeditor");
        }else{
        	$this->path = Yii::app()->getAssetManager()->publish(dirname(__FILE__).DIRECTORY_SEPARATOR."ckeditor");
        }    
   ...
   private function recursiveChmod($path, $filePerm=0644, $dirPerm=0755)
   {
    ...
    //print_r($this);
        $this->render($this->type, array(
                ...
                'path'=>$this->path,
                'publishPath' => $this->path/*Yii::app()->getAssetManager()->getPublishedPath(dirname(__FILE__).DIRECTORY_SEPARATOR."fckeditor")*/
            ));
    }
}
?>

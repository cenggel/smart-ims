...
return array (
  ...
  'Here you can view and manage the permissions assigned to each role.'=>'这里能管理赋给每个角色的权限。',
  'Authorization items can be managed under {roleLink}, {taskLink} and {operationLink}.'=>'权限项目可以在{roleLink}, {taskLink} 和 {operationLink}管理。',
  'Generate items for controller actions'=>'从控制器生成授权项目',
  'Here you can view which permissions has been assigned to each user.'=>'在这里你可以看到赋给每位用户的权限。',
  'A role is group of permissions to perform a variety of tasks and operations, for example the authenticated user.'=>'角色是一组任务和操作的组合。',
  'Roles exist at the top of the authorization hierarchy and can therefore inherit from other roles, tasks and/or operations.'=>'角色在权限数的顶端，可以继承自其他角色、任务后操作。',
  'A task is a permission to perform multiple operations, for example accessing a group of controller action.'=>'任务是一组操作的组合，例如允许访问一个控制器的一组动作。',
  'Tasks exist below roles in the authorization hierarchy and can therefore only inherit from other tasks and/or operations.'=>'任务是权限层级关系中在角色下面，只允许继承从其他任务或操作。',
  'An operation is a permission to perform a single operation, for example accessing a certain controller action.'=>'操作是权限中最小单位，只允许一种动作。',
  'Operations exist below tasks in the authorization hierarchy and can therefore only inherit from other operations.'=>'操作在权限层级关系中在任务下面，只允许继承其他操作。',
);
